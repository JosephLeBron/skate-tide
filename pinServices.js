...
app.post('/api/delete-pin', (req, res) => {
    try {
        ...
        const stmt = db.prepare('DELETE FROM pins WHERE name = ?');
        const result = stmt.run(name, lat, lon, rating, picture, difficulty);

        if (result.changes > 0) {
            console.log(`Deleted pin with name ${name}`);
            res.status(200).json('Pin deleted successfully');
        } else {
            console.log(`Pin with name ${name} not found, skipping deletion.`);
